<div class="modal inactive modal-days">
    ...
    <div class="modal-info">
        <h2 class="modal-title">День № {{$Day->day}}</h2>
    </div>
    <form class="modal__form">
        <div class="modal__form-item">
            <label for="mail">Название</label>
            <input type="text" name="email" required="required" value="{{$Day->name}}">
        </div>
        <div class="modal__form-item">
            <label for="mail">Че то</label>
            <input type="password" name="password" required="required" value="{{$Day->description}}">
        </div>
        <div class="modal__form-item">
            <label for="mail">То-то</label>
            <input type="password" name="password-confirm" required="required" value="{{$Day->longread}}">
        </div>
    </form>
    <button id="submit" name="{{$Day->id}}">Изменить</button>
</div>
