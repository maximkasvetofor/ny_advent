...
<div class="modal inactive modal-login">
    ...
    <form class="modal__form" id="login-form" method="GET" action="{{route('advent.auth')}}">
        @csrf
...
<script>
    ...
    loginForm.addEventListener('submit', async (event) => {
        ...
        const responseLogin = await fetch(loginForm.action, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': csrfTokenLogin,
            },
            body: formDataLogin,
        });
        if (!responseLogin.ok) {
